...
public class CraftingInterfaceScreen extends AbstractContainerScreen<CraftingInterfaceMenu> {
    ...
    public boolean canScroll() {
        return Minecraft.getInstance().level.getBlockEntity(menu.getPos()) instanceof CraftingInterfaceBE be && be.getInvSize() > 45;
    }
    ...
    private int calculateRowCount() {
        if (Minecraft.getInstance().level.getBlockEntity(menu.getPos()) instanceof CraftingInterfaceBE be) return (int) Math.ceil(be.getInvSize() / 9.0) - 5;
        return 0;
    }
